defmodule MoisesAI.API do

  def get(url, query \\ []) do
    HTTPoison.start
    query = MoisesAI.Utils.encode_body(query)

    unless String.length(query) == 0 do
      url = "#{url}?#{query}"
    end

    HTTPoison.get!(url, MoisesAI.auth_header)
    |> handle_response
  end
  ...
  def post(url, body) do
    ...
    HTTPoison.post!(url, req_body, MoisesAI.auth_header)
    |> handle_response
  end

  def delete(url) do
    HTTPoison.start
    HTTPoison.delete!(url, MoisesAI.auth_header)
    |> handle_response
  end
